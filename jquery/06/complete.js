$(document).ready(function () {
  ...
  $('div.letter').mouseover(function () {
    var idName = $(this).attr('id');
    $('#dictionary').load('does-not-exist.html #' + idName, function (response, status, jqXHR) {
      if (status == 'error') {
        console.log('data');
        $('#dictionary')
          .html('An error occurred: ' + jqXHR.status + ' and:' + jqXHR.responseText);
      }
    });
  });
  ...
  $.getJSON(url, function(data) {
    ...
    $.each(data, function(index, entry) {
      ...
      html += '<h3 class="url">' + entry.url + '</h3></div>';
    });
    $('#dictionary').html(html);
  });
});